(() => {
  ...
  const reviewBox = app.querySelector('#vdh-push-review');
  const reviewButton = app.querySelector('#vdh-review-push');
  const confirmButton = app.querySelector('#vdh-confirm-push');
  ...
  const renderItems = (items) => {
    ...
    list.innerHTML = items.map((item) => {
      ...
      const fileMeta = formatSize(item.size);
      return `
        <button type="button" class="vdh-browser-item" data-type="${escapeHtml(item.type)}" data-path="${escapeHtml(item.path)}">
          ...
          <span class="vdh-browser-meta">${isDir ? 'Folder' : `Select file${fileMeta ? ` · ${escapeHtml(fileMeta)}` : ''}`}</span>
  ...
  if (reviewButton) {
    reviewButton.addEventListener('click', (event) => {
      ...
      clearReviewFeedback();
      if (reviewBox) {
        reviewBox.innerHTML = '<div class="vdh-warning">Select a file first using Browse files, or enter a valid file path.</div>';
      }
      pathInput.focus();
    }, true);
  }
