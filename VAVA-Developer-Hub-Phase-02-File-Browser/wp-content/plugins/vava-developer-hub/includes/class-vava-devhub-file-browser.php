...
final class Vava_DevHub_File_Browser {
    ...
    private static function validate_browse_path($repository, $path) {
        ...
        if ($repository === 'Vava-living-website' && $path !== '') {
            ...
            if (!$allowed) {
                return new WP_Error('outside_safe_scope', 'File browser is limited to the Vava custom theme and Developer Hub plugin.');
            }
        }
    ...
    private static function virtual_roots() {
        return array(
            array(
                'name' => 'Vava Theme',
            ...
            array(
                ...
                'path' => 'wp-content/plugins/vava-developer-hub',
                'type' => 'dir',
                'size' => 0,
                'sha' => '',
            ),
