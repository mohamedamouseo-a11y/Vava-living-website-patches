...
final class Vava_DevHub_GitHub {
    ...
    public static function connection() {
        if (!self::has_token()) {
            return array(
                ...
                'authenticated' => false,
                'login' => '',
                'message' => 'Read-only public GitHub access. Add VAVA_GITHUB_TOKEN for authenticated access and controlled push.',
            );
        }
    ...
    public static function controlled_push($repository, $branch, $path, $content, $message, $expected_sha = '') {
        if (!self::has_token()) {
            return new WP_Error('token_required', 'VAVA_GITHUB_TOKEN is required for controlled push.');
        }
        $repository = Vava_DevHub_Security::sanitize_repository($repository);
        $branch = Vava_DevHub_Security::sanitize_branch($branch);
